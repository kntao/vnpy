...
TThostFtdcInstrumentIDType INSTRUMENT_ID = "rb1910";					// 交易合约代码,必须和CSV数据文件中合约字段代码一致
...
char *ppInstrumentID[] = { "rb1910"};			// 行情订阅列表，交易合约代码,必须和CSV数据文件中合约字段代码一致，目前只支持1个合约同时回测，不支持订阅多个合约回测
